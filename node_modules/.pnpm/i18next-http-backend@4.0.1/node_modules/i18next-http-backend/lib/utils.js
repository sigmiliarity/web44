...
// oversized inputs. The dimensions that differ per-key are whether `/` and
// `@` are allowed — see the two per-key helpers below.
function isSafeUrlSegmentBase (v) {
  ...
  // of `..`/`/` via `%2E%2E`/`%2F`), space (ambiguous).
...
// Strict — also rejects `/` and `@`. Applied to `lng`:
...
// contains `/` or `@` (`@` is the authority boundary in
// userinfo-containing URLs), so allowing them would only enable attacks.
...
// Loose — allows `/` and `@`. Applied to `ns`:
// nested namespace names like `a/b` that map to subfolder URL layouts
// (`/locales/en/a/b.json`) and scoped-package-style names like
// `@org/package-name` are documented/common i18next patterns and were
// rejected in 3.0.5 as an unintended regression. In the URL path position
// both characters are plain RFC 3986 pchars with no structural meaning.
// Directory escape is still prevented — `..` is still blocked, `\` is
// still blocked, URL-structure characters are still blocked — so the
// security fix remains in force for every concrete attack pattern from
